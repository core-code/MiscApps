...
static char JMSUAppleQuarantineIdentifier[] = "com.apple.quarantine";

static BOOL JMSUMakeRefFromURL(NSURL *url, FSRef *ref, NSError **error) {
...
@implementation JMSUFileManager
{
...
- (int)releaseItemFromQuarantineAtRootURL:(NSURL *)rootURL error:(NSError *__autoreleasing *)error
{
    static const int removeXAttrOptions = XATTR_NOFOLLOW;
    BOOL success = -1; // no quaran tine
    
    // First remove quarantine on the root item
    NSString *rootURLPath = rootURL.path;
    if ([self _getXAttr:JMSUAppleQuarantineIdentifier fromFile:rootURLPath options:removeXAttrOptions] >= 0)
    {
        success = 1;
        
        BOOL removedRootQuarantine = ([self _removeXAttr:JMSUAppleQuarantineIdentifier fromFile:rootURLPath options:removeXAttrOptions] == 0);
    ...
    if ([rootType isEqualToString:NSFileTypeDirectory]) {
        ...
        for (NSURL *fileURL in directoryEnumerator) {
            if ([self _getXAttr:JMSUAppleQuarantineIdentifier fromFile:fileURL.path options:removeXAttrOptions] >= 0) {
                
                if (success == -1)
                    success = 1;

                BOOL removedQuarantine = ([self _removeXAttr:JMSUAppleQuarantineIdentifier fromFile:fileURL.path options:removeXAttrOptions] == 0);
...
- (BOOL)copyItemAtURL:(NSURL *)sourceURL toURL:(NSURL *)destinationURL error:(NSError * __autoreleasing *)error
{
    ...
    FSRef sourceRef;
    if (!JMSUMakeRefFromURL(sourceURL, &sourceRef, error)) {
        return NO;
    }

    FSRef destinationParentRef;
    if (!JMSUMakeRefFromURL(destinationURL.URLByDeletingLastPathComponent, &destinationParentRef, error)) {
        return NO;
    }
...
{
    FSRef pathRef;
    if (!JMSUMakeRefFromURL(url, &pathRef, NULL)) {
        return NO;
    }
...
- (BOOL)_updateItemAtURL:(NSURL *)targetURL withAccessTime:(struct timeval)accessTime error:(NSError * __autoreleasing *)error
{
    ...
    // WARNING: this is new code to prevent permission problems after updating apps with damaged permissions without group/others read bit set
    mode_t newmode;
    newmode = getmode(setmode("+r+X"), statInfo.st_mode); // give everyone read permissions, give everyone execute, if anyone has execute
    fchmod(fileDescriptor, newmode);
    // END WARNING
...
- (BOOL)updateAccessTimeOfItemAtRootURL:(NSURL *)targetURL error:(NSError * __autoreleasing *)error
{
    ...
    if (![self _updateItemAtURL:targetURL withAccessTime:currentTime error:error]) {
        success =  NO; // WARNING: new code not to return here because we want to (try to) update permissions in any case
    }
