...
// + * structure, too.
// + */
//int darwin_legacy_getdirentries(int, char *, int, long *)
//__asm("_getdirentries");
...
//	struct stat	sb;
    ...
    ebuf = buf = emalloc(direntnum * sizeof(struct dirent));
    while ((dp = readdir(dirp)) != NULL)
    {
        memcpy(ebuf, dp, sizeof(struct dirent));
        ebuf += sizeof(struct dirent);
    }
...
//	close(fd);
	...
	for (entries = 0, cp = buf; cp < ebuf; ) {
		...
		if (dp->d_reclen <= 0)
			break;
		cp += sizeof(struct dirent);//dp->d_reclen;
	}
//    entries = direntnum;
//    dirlist = emalloc(sizeof(struct dirent *) * (entries + 1));
//    memcpy(dirlist, buf, entries * sizeof(struct dirent));
...
diffit(struct dirent *dp, char *path1, size_t plen1, char *path2, size_t plen2)
{
	...
	if (S_ISDIR(stb1.st_mode) && S_ISDIR(stb2.st_mode)) {
		if (rflag)
			...
			fprintf(buffer, "Common subdirectories: %s and %s\n",
			    path1, path2);
		return;
	}
